...
export async function POST() {
  try {
    ...
    if (existingCount.count > 0) {
      return NextResponse.json({ message: 'Preset templates already exist', count: existingCount.count });
    }
    ...
    return NextResponse.json({ message: 'Preset templates seeded successfully', count: templates.length });
  } catch (error) {
